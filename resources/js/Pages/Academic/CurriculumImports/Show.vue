...
type Proposal = {
    ...
    parser_note: string | null; confidence: number | null; manually_edited: boolean; warnings: string[]; children?: Proposal[];
};
type Period = Proposal & { children: Proposal[] };
...
const allProposals = computed(() => props.periods.flatMap(flatten));
    ...
    <AuthenticatedLayout>
        ...
        <form @submit.prevent="save">
            <section v-for="period in periods" :key="period.id" class="card mb-4" :aria-labelledby="`period-${period.id}`">
                <div class="card-header bg-white"><div class="row g-2 align-items-end">
