...
<template>
    ...
    <AuthenticatedLayout>
        <div class="d-flex justify-content-between align-items-start gap-3 mb-3">
            <div><h1 class="h2 mb-1">{{ source.title }}</h1><p class="text-secondary mb-0">{{ label(source.source_category) }} · {{ label(source.source_kind) }} · {{ label(source.authority_level) }}</p></div>
            <div class="d-flex gap-2"><Link class="btn btn-outline-secondary" :href="route('academic.sources.index')">Back</Link><Link v-if="permissions.manage && !source.archived_at" class="btn btn-outline-primary" :href="route('academic.sources.edit', source.id)">Edit</Link></div>
        </div>
        <AcademicNav />
        ...
        <div class="row g-4"><div class="col-lg-8">
            <section v-if="calendarSetup.is_calendar" class="card border-primary mb-4" aria-labelledby="calendar-setup-heading"><div class="card-body">
                ...
                <template v-else>
                    ...
                    <p v-else-if="source.review_status === 'reviewed'" class="text-secondary">The source is reviewed and ready to create an empty draft profile. No calendar events will be extracted.</p>
