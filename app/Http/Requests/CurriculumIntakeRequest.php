...
class CurriculumIntakeRequest extends FormRequest
{
    ...
    public function rules(): array
    {
        ...
        return [
            'student_id' => ['required', 'integer', Rule::exists('students', 'id')->where('tenant_id', $tenantId)->where('status', 'active')],
            'school_year_id' => ['required', 'integer', Rule::exists('school_years', 'id')->where('tenant_id', $tenantId)],
            'source_origin' => ['required', Rule::in(['provider', 'publisher', 'custom', 'other'])],
            'education_provider_id' => ['nullable', 'required_if:source_origin,provider,publisher', 'prohibited_unless:source_origin,provider,publisher', $this->visibleExists('education_providers')],
            'subject_id' => ['required', 'integer', $this->visibleExists('subjects')],
