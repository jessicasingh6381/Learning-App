...
The command reports source and target counts before either mode. The dry run verifies drivers, PostgreSQL version, target name, logical schema parity, and an empty target. Logical schema parity requires identical application-column sets and compatible types, lengths/precision, nullability, defaults, and generated/sequence behavior. It deliberately ignores physical column order because Laravel's MySQL `after()` placement hints are not implemented by PostgreSQL. The comparator recognizes only audited engine mappings such as unsigned MySQL integers to PostgreSQL signed integers, `tinyint(1)` to boolean, MariaDB `longtext` JSON storage to PostgreSQL `json`, `char(36)` UUID storage to PostgreSQL `uuid`, and MySQL `AUTO_INCREMENT` to PostgreSQL sequences. Missing or unexpected columns still fail preflight.

The dry run reads and transforms every source row, validates integer ranges, UUIDs, JSON, and dates, and performs no target writes. Execution preserves primary keys, defers and restores self-referencing keys, converts MySQL `0`/`1` values for PostgreSQL booleans, validates JSON without changing valid JSON text, preserves nulls/text/decimals/timestamps and string case exactly, rejects zero dates, resets ID sequences, and validates before commit. No rows are intentionally skipped. Any exception rolls back the target transaction.
