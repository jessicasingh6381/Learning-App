...
### Calendar Profile lifecycle

`CalendarProfileLifecycleService` is the authoritative boundary for dependency inspection, archival, restoration, and permanent deletion. It returns safe counts for current/historical configurations, events, and linked sources; usage labels; action capabilities; and human-readable blockers. Controllers and Vue consume those results rather than duplicating lifecycle decisions.

Normal status editing permits Draft to Active, Active to Retired, and Retired to Active, including no-op saves in the current status. Archival is deliberately excluded from the generic edit form. The dedicated Archive action accepts Draft, Active, or Retired profiles only when no Draft or Active Academic Setup configuration selects them. Closed or archived configuration references are historical and permit archival. Archive updates only status, writes an audit, and preserves events, direct URLs, source links, configuration references, and prior audits. Archived profiles are read-only: event and source-link mutations are rejected until the dedicated Restore action returns the profile to Draft. Restoration does not select, clear, or otherwise mutate an Academic Setup configuration.

The default index includes Draft and Active profiles. Archived and All filters expose retained records; retired profiles appear under All. Every list/detail response identifies usage without exposing internal configuration identifiers.

Permanent deletion is not a status transition. A tenant owner or administrator with `calendars.manage` may delete only an active-tenant-owned, non-shared profile with zero Calendar Events, zero Academic Source links, and zero Academic Year Configuration references of any lifecycle status. The request requires an explicit `DELETE` confirmation. Inside a transaction, the service locks the profile row, recomputes every dependency, writes a minimal audit containing only name, academic-year label, and status, then deletes only the profile. Existing restrictive foreign keys remain intact; there is no delete-with-events or cascading source cleanup. Tenant-scoped binding hides foreign profiles, shared profiles fail policy authorization, and students never reach administrative routes.
